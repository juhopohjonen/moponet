...
from flask import Flask, jsonify, abort, redirect, render_template, url_for, session, send_from_directory, request
...
def create_user (username, email, unhashed_pwd, isDiscordUser):
    # Create password hash and user object with it
    User = models.User
    
    password_hash = generate_password_hash(unhashed_pwd)
    
    current_user = User(username=username, email=email, password=password_hash, isDiscordUser=isDiscordUser)
...
def process_login_user(request_form, error_page, r):
    ...
    # (do not include discord records) 

    current_user = models.User.query.filter_by(
            username=username, isDiscordUser=0).first()
    ...
    if request_form.get('remember') == 'on':
        r.permanent = True

    print(f'remember: {request_form.get("remember")}, type: {type(request_form.get("remember"))}')
